        ...
        if savepath:
            ...
            if save_format == 'nii':
                ...
                if verbose:
                    ...
                    logging.info(''.join(['1:', str(np.sum(seg_img == 1)), ' | 2:', str(np.sum(seg_img == 2)), ' | 4:', str(np.sum(seg_img == 4))]))
                    logging.info(''.join(['WT:', str(np.sum((seg_img == 1) | (seg_img == 2) | (seg_img == 4))), ' | TC:', str(np.sum((seg_img == 1) | (seg_img == 4))), ' | ET:', str(np.sum(seg_img == 4))]))
        ...
        if scoring:
            ...
            if snapshot:
                ...
                for frame in range(T):
                    ...
                    #os.makedirs(os.path.join('snapshot1', cfg, name), exist_ok=True)
                    imageio.imwrite(os.path.join('snapshot',cfg, name, str(frame) + '.png'), Snapshot_img[:,:,:,frame])
                    #imageio.imwrite(os.path.join('snapshot1', cfg, name, str(frame) + '.png'),Snapshot_img[:, :, frame, :])
